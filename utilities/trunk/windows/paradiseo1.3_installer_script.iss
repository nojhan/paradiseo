...
#define OutputPath="C:\Documents and Settings\tlegrand\Bureau\TestInstalleur\dest"

// installer source info
#define ParadiseoSourceTag="E:\software\paradisEO\repository\tags\paradiseo-1.3-beta2\*"
#define EoTag="E:\software\eo\repository\tag2_1_3\*"

// installer description info
#define Version="1.3"
#define VersionMain="ParadisEO-1.3-beta"
#define InstallerName="paradiseo-1.3-installer"
#define ApplicationName="ParadisEO-1.3-beta"
...
#define LicenceFile="E:\software\paradisEO\repository\tags\paradiseo-1.3-beta\LICENSE"
...
#define AboutUS="INRIA Lille - Nord Europe, Dolphin Project-team"
...
DefaultDirName={userdesktop}\{#ApplicationName}
DefaultGroupName={#ApplicationName}
//InfoAfterFile=E:\software\paradisEO\repository\tags\paradiseo1.3\afterInstall.txt
...
english.Advanced=(for advanced user)
...
french.Advanced=(pour utilisateur confirmé)
...
Source: E:\software\eo\repository\tmp\paradiseo-eo\CMakeLists.txt; DestDir: {app}\paradiseo-eo; Excludes: """*.~*"""; Flags: ignoreversion recursesubdirs createallsubdirs
Source: E:\software\eo\repository\tmp\paradiseo-eo\src\utils\pipecom.h; DestDir: {app}\paradiseo-eo\src\utils; Excludes: """*.~*"""; Flags: ignoreversion recursesubdirs createallsubdirs
Source: E:\software\eo\repository\tmp\paradiseo-eo\src\utils\pipecom.cpp; DestDir: {app}\paradiseo-eo\src\utils; Excludes: """*.~*"""; Flags: ignoreversion recursesubdirs createallsubdirs
Source: E:\software\eo\repository\tmp\paradiseo-eo\src\eoCtrlCContinue.h; DestDir: {app}\paradiseo-eo\src; Excludes: """*.~*"""; Flags: ignoreversion recursesubdirs createallsubdirs
Source: E:\software\eo\repository\tmp\paradiseo-eo\src\eoCtrlCContinue.cpp; DestDir: {app}\paradiseo-eo\src; Excludes: """*.~*"""; Flags: ignoreversion recursesubdirs createallsubdirs
...
begin
    ...
    begin
	 ...
	 if GeneratorBox.Checked[13] then
    begin
            Generator:='Eclipse CDT4 - MinGW Makefiles' ;
            exit;
    end;
     if GeneratorBox.Checked[14] then
    begin
...
//  MinConfig: String;
begin
    //CTestConfig:=' -D ExperimentalStart -D ExperimentalBuild -D ExperimentalTest' ;
	CTestConfig:=' -D ExperimentalStart -D ExperimentalBuild' ;
    ...
    begin
            ...
            CMakeAdditionalTags:= ' -DENABLE_MINIMAL_CMAKE_TESTING=FALSE -DENABLE_CMAKE_TESTING=FALSE';
            exit;
    end;
    if BuildModeBox.Checked[2] then
    begin
            //CMakeAdditionalTags:= ' -DCMAKE_BUILD_TYPE=Debug -DENABLE_CMAKE_TESTING=TRUE -DENABLE_MINIMAL_CMAKE_TESTING=FALSE';
            CMakeAdditionalTags:= ' -DCMAKE_BUILD_TYPE=DEBUG -DENABLE_CMAKE_TESTING=TRUE -DENABLE_MINIMAL_CMAKE_TESTING=FALSE';
...
begin
  ...
  begin
	...
	if (GeneratorBox.Checked[1] or GeneratorBox.Checked[3]) then
        	begin
         		CreateShellLink(ExpandConstant('{userdesktop}\eo.lnk'), 'Opens the licence database in SQLite', ExpandConstant('{app}\paradiseo-eo\build\EO.sln'), '', ExpandConstant('{userdesktop}'), '', 0, SW_SHOWNORMAL);
		end;
        ...
        if IsComponentSelected('mo') then
         begin
          ...
          updateProgressBar(50,60);
           ...
           if (GeneratorBox.Checked[1] or GeneratorBox.Checked[3]) then
        	begin
         		CreateShellLink(ExpandConstant('{userdesktop}\mo.lnk'), 'Opens the licence database in SQLite', ExpandConstant('{app}\paradiseo-mo\build\ParadisEO-MO.sln'), '', ExpandConstant('{userdesktop}'), '', 0, SW_SHOWNORMAL);
		end;
         end else
            updateProgressBar(50,60);
        ...
        if IsComponentSelected('moeo') then
         begin
          ...
          updateProgressBar(60,75);
           ...
           if (GeneratorBox.Checked[1] or GeneratorBox.Checked[3]) then
        	begin
         		CreateShellLink(ExpandConstant('{userdesktop}\moeo.lnk'), 'Opens the licence database in SQLite', ExpandConstant('{app}\paradiseo-moeo\build\ParadisEO-MOEO.sln'), '', ExpandConstant('{userdesktop}'), '', 0, SW_SHOWNORMAL);
		end;
...
begin
  ...
  GeneratorBox.AddRadioButton('Eclipse CDT4 - MinGW Makefiles', '', 0, False, True, nil);
  GeneratorBox.AddRadioButton('Eclipse CDT4 - NMake Makefiles', '', 0, False, True, nil);
  ...
  BuildModeBox.AddRadioButton('+ Tests ' + CustomMessage('Advanced'), '', 0, False, True, nil);
